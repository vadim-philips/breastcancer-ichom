...
* enableWhen
  ...
  * operator = #=
  * answerCoding = {code}
* enableBehavior = #any // logically unnecessary, but smartqedit4 requires it, so let's add for demo purposes
...
* item[+]
  * linkId = "Group"
  * text = "SACQ comorbidities questionnaire"
  * type = #group // unnecessary, but smartqedit4 requires it, so let's add for demo purposes
